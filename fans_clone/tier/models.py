...
class Tier(models.Model):
	...
	def __str__(self):
		return "Tier" + str(self.number) + '--' + str(self.user) + ' - ' + self.description
...
class Subscription(models.Model):
	subscriber = models.ForeignKey(User, on_delete=models.CASCADE,null=True,blank=True, related_name='subscriber')
	subscribed = models.ForeignKey(User, on_delete=models.CASCADE,null=True,blank=True, related_name='subscribed')
	tier = models.ForeignKey(Tier, on_delete=models.CASCADE, related_name='tier')
	date = models.DateTimeField(auto_now_add=True)
	expired = models.BooleanField(default=False)

	def __str__ (self):
		return  self.subscribed.username + 'Tier' + str(self.tier.number)
