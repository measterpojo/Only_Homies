...
from .models import Profile, PeopleList
...
def UserProfile(request, username):
	...
	template = loader.get_template('authy/profile.html')
...
def addToList(request):
	user = request.user
	if request.method == 'POST':
		to_list = request.POST.get('listed_item')
...
def showList(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			title = form.cleaned_data.get('title')
			PeopleList.objects.create(title=title, user=request.user)
			return HttpResponseRedirect(reverse('show-my-list'))
	else:
		form = NewListForm()
	...
	return render(request, 'authy/user_lists.html', context)

def peopleList(request, list_id):
	user_lists = get_object_or_404(PeopleList, id=list_id)

	context = {
		'user_lists':user_lists,
	}

	return render(request,'authy/people_list.html', context)


def peoplelistdelete(request, list_id):
	PeopleList.objects.filter(id=list_id).delete()

	return HttpResponseRedirect(reverse('profile', args=['show-my-list']))


def removeFromList(request, username):
	person = get_object_or_404(User, username=username)
	list_id = PeopleList.objects.get(user=request.user, people=people)
	try:
		PeopleList.people.through.objects.filter(user__id=person.id, peoplelist_id=list_id).delete()
