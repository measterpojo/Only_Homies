...
def ForbiddenUsers(value):
	...
	if value.lower() in forbidden_users:
		raise ValidationError('Forbidden name, choose another!!!')


def InvalidUser(value):
	if '@' in value or '+' in value or '-' in value or '=' in value:
		raise ValidationError('Invalid Charaters: @, +, =, -')

def UniqueEmail(value):
	if User.objects.filter(email__iexact=vaule).exists():
...
class SignupForm(forms.ModelForm):
	username = forms.CharField(widget=forms.TextInput(), max_length=40, required=True)
	email = forms.CharField(widget=forms.EmailInput(), max_length=50, required=True)
	password = forms.CharField(widget=forms.PasswordInput())
	confirm_password = forms.CharField(widget=forms.PasswordInput(), required=True, label=
...
class ChangePasswordForm(forms.ModelForm):
	...
	confirm_password = forms.CharField(widget=forms.PasswordInput(attrs={'class':'input is-medium'}), label='Old password', required=True)
	...
	def clean(self):
		...
		if not user.check_password(old_password):
			self.errors['old_password'] = self.error_class(['Old password didnt match our records'])
